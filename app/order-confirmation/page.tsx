...
export default function OrderConfirmationPage() {
      ...
      const fetchPaymentDetails = async () => {
        try {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/stripe/payment-intents/${paymentIntentId}`,
            {
              method: "GET",
              headers: {
  ...
  const handlePaymentSuccess = async (paymentIntentId: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/stripe/confirm-payment`, {
        method: "POST",
        headers: {
